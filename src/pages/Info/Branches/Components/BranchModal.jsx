...
export default function BranchModal({ open, onCancel, onSave, initialData, branchTypes }) {
    ...
    useEffect(() => {
        if (initialData) {
            form.setFieldsValue({
                ...
                open_time: initialData.open_time ? moment(initialData.open_time, "HH:mm") : null,
                close_time: initialData.close_time ? moment(initialData.close_time, "HH:mm") : null,
            });
        } else {
            form.resetFields();
        }
        ...
        form.validateFields()
            .then((values) => {
                const payload = {
                    ...
                    open_time: values.open_time ? values.open_time.format("HH:mm") : null, // Format time
                    close_time: values.close_time ? values.close_time.format("HH:mm") : null, // Format time
                };
    ...
    return (
        ...
        >
            <Form form={form} layout="vertical">
                <Form.Item name="branch_code" label="Mã chi nhánh" rules={[{ required: true, message: "Vui lòng nhập mã chi nhánh!" }]}>
                    <Input placeholder="Nhập mã chi nhánh" disabled={!!initialData?.id} />
                </Form.Item>
                ...
                <Form.Item
                    ...
                    rules={[{ required: true, message: "Vui lòng nhập ngày hoạt động!" }]}
                >
                    <Input placeholder="Ví dụ: Thứ 2 - Chủ nhật" />
                </Form.Item>
                <Form.Item name="open_time" label="Giờ mở cửa" rules={[{ required: true, message: "Vui lòng nhập giờ mở cửa!" }]}>
                    <TimePicker format="HH:mm" />
                </Form.Item>
                <Form.Item name="close_time" label="Giờ đóng cửa" rules={[{ required: true, message: "Vui lòng nhập giờ đóng cửa!" }]}>
                    <TimePicker format="HH:mm" />
                </Form.Item>
                <Form.Item
                    name="manager_name"
                ...
                <Form.Item
                    ...
                    rules={[{ required: true, message: "Vui lòng nhập số điện thoại người quản lý!" }]}
                >
                    <Input placeholder="Nhập số điện thoại" />
                </Form.Item>
                <Form.Item
                ...
                <Form.Item name="description" label="Mô tả">
                    <Input.TextArea rows={3} placeholder="Nhập mô tả chi nhánh" />
                </Form.Item>
            </Form>
        </Modal>
    );
