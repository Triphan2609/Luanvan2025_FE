...
export default function ServiceManagement() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const [servicesData, branchesData, serviceTypesData] =
                    await Promise.all([
                        getServices(),
                        getBranches(),
                        getServiceTypes(),
                ...
                // Lọc dịch vụ chỉ thuộc chi nhánh nhà hàng
                setServices(
                    servicesData.filter((s) =>
                        restaurantBranches.some(
                            (b) => b.id === s.branch_id || b.id === s.branchId
                        )
                    )
                );
    ...
    const handleStatusChange = async (id, isActive) => {
        ...
        try {
            ...
            message.success(
                `Dịch vụ đã được ${isActive ? "kích hoạt" : "ngừng hoạt động"}`
            );
        } catch (error) {
    ...
    const filteredServices = useMemo(() => {
        return services
            ...
            .filter((s) =>
                filterBranch ? s.branch_id === filterBranch : true
            );
    ...
    const columns = [
        ...
        },
        {
            ...
            key: "serviceType",
            render: (_, record) => {
                return <span>{record.serviceType?.name || "-"}</span>;
            },
            sorter: (a, b) =>
                (a.serviceType?.name || "").localeCompare(
                    b.serviceType?.name || ""
                ),
        },
        {
    ...
    const handleExport = () => {
        try {
            const formattedData = services.map((service) => ({
                ...
                "Loại dịch vụ":
                    service.serviceType?.name === "HOTEL"
                        ? "Khách sạn"
                        : service.serviceType?.name === "RESTAURANT"
                        ? "Nhà hàng"
                        : "-",
    ...
    return (
        <Tabs defaultActiveKey="1">
            <Tabs.TabPane tab="Quản lý Dịch vụ & Tiện ích" key="1">
                <Card
            ...
            <Tabs.TabPane tab="Quản lý Loại Dịch vụ & Tiện ích" key="2">
